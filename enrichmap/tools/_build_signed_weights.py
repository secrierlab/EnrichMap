...
def build_signed_weights(
    ...
    CV-inferred weights capture spatial variability, while the sign encodes
    biological direction. Each group is normalised by its own size so that
    ...
    up_present = [g for g in up_set if g in cv_weights]
    down_present = [g for g in down_set if g in cv_weights]

    n_up = len(up_present)
    n_down = len(down_present)

    if n_up == 0 and n_down == 0:
        raise ValueError("No genes from either list found in the dataset.")

    weights = {}
    for g in up_present:
        weights[g] = cv_weights[g] / n_up
    for g in down_present:
        weights[g] = -cv_weights[g] / n_down
